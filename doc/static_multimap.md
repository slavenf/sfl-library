...
`sfl::static_multimap` can be used in C++20 constant expressions.
...
* On GCC, it is not possible to declare `constexpr` objects of `static_multimap`. Clang and MSVC allow this, so this is a compiler-specific limitation (possibly a bug).
