...
`sfl::small_unordered_flat_set` is an unordered associative container similar
to `std::unordered_set`.
It contains set of unique keys.
Comparison is done according to the comparison function `KeyEqual`.

Unlike `std::unordered_set`, the elements are stored contiguously in the memory.
...
`sfl::small_unordered_flat_set` holds statically allocated array of size `N` in
the object itself.
...
`sfl::small_unordered_flat_set` meets the requirements of
