...
`sfl::static_unordered_map` meets the requirements of [*Container*](https://en.cppreference.com/w/cpp/named_req/Container) and [*UnorderedAssociativeContainer*](https://en.cppreference.com/w/cpp/named_req/UnorderedAssociativeContainer).

`sfl::static_unordered_map` can be used in C++20 constant expressions.
...
* On GCC, it is not possible to declare `constexpr` objects of `static_unordered_map`. Clang and MSVC allow this, so this is a compiler-specific limitation (possibly a bug).
...
1.  ```
    ...
    **Effects:**
    Returns the maximum number of elements the container is able to hold, i.e. `StaticCapacity`.
...
1.  ```
    ...
    **Effects:**
    Returns the number of buckets in the container, i.e. returns `StaticBucketCount`.
...
1.  ```
    ...
    **Effects:**
    Returns the maximum number of buckets the container is able to hold due to system or library implementation limitations, i.e. returns `StaticBucketCount`.
...
1.  ```
    ...
    **Effects:**
    Returns current maximum load factor, i.e. `StaticCapacity` divided by `StaticBucketCount`.
