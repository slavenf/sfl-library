# sfl::static_unordered_linear_multiset
...
Defined in header `sfl/static_unordered_linear_multiset.hpp`:
...
* On GCC, it is not possible to declare `constexpr` objects of `static_unordered_linear_multiset`. Clang and MSVC allow this, so this is a compiler-specific limitation (possibly a bug).
...
3.  ```
    static_unordered_linear_multiset& operator=(std::initializer_list<Key> ilist);
    ```
