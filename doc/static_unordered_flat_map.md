...
`sfl::static_unordered_flat_map` is an associative container similar to [`std::unordered_map`](https://en.cppreference.com/w/cpp/container/unordered_map), but the capacity is **fixed** and the underlying storage is implemented as a **unsorted vector**.
