...
Defined in header `sfl/flat_multiset.hpp`:
...
`sfl::flat_multiset` is an associative container that contains a sorted collection of keys, where multiple elements can have equivalent keys. Underlying storage is implemented as a sorted [`vector`](vector.md), providing a compact and cache-friendly representation.
...
1.  ```
    ...
    ```
2.  ```
    explicit flat_multiset(const Compare& comp) noexcept(
        std::is_nothrow_default_constructible<Allocator>::value &&
...
3.  ```
    ...
    ```
4.  ```
    explicit flat_multiset(const Compare& comp, const Allocator& alloc) noexcept(
        std::is_nothrow_copy_constructible<Allocator>::value &&
        std::is_nothrow_copy_constructible<Compare>::value
    );
    ```
...
6.  ```
    template <typename InputIt>
    flat_multiset(InputIt first, InputIt last, const Compare& comp);
    ```
7.  ```
    template <typename InputIt>
...
8.  ```
    template <typename InputIt>
    flat_multiset(InputIt first, InputIt last, const Compare& comp, const Allocator& alloc);
    ```
...
9.  ```
    ...
    ```
10. ```
    flat_multiset(std::initializer_list<value_type> ilist, const Compare& comp);
    ```
11. ```
    flat_multiset(std::initializer_list<value_type> ilist, const Allocator& alloc);
    ```
12. ```
    flat_multiset(std::initializer_list<value_type> ilist, const Compare& comp, const Allocator& alloc);
    ```
...
17. ```
    ...
    flat_multiset(sfl::from_range_t, Range&& range);
    ```
18. ```
    template <typename Range>
    flat_multiset(sfl::from_range_t, Range&& range, const Compare& comp);
    ```
19. ```
    template <typename Range>
    flat_multiset(sfl::from_range_t, Range&& range, const Allocator& alloc);
    ```
20. ```
    template <typename Range>
    flat_multiset(sfl::from_range_t, Range&& range, const Compare& comp, const Allocator& alloc);
    ```
...
3.  ```
    flat_multiset& operator=(std::initializer_list<Key> ilist);
    ```
...
1.  ```
    ...
    bool operator==
    (
        const flat_multiset<K, C, A>& x,
        const flat_multiset<K, C, A>& y
    );
    ```
...
1.  ```
    ...
    void swap
    (
        flat_multiset<K, C, A>& x,
        flat_multiset<K, C, A>& y
    );
    ```
...
1.  ```
    ...
    typename flat_multiset<K, C, A>::size_type
        erase_if(flat_multiset<K, C, A>& c, Predicate pred);
    ```
