...
`sfl::static_flat_multiset` can be used in C++20 constant expressions.
...
* On GCC, it is not possible to declare `constexpr` objects of `static_flat_multiset`. Clang and MSVC allow this, so this is a compiler-specific limitation (possibly a bug).
