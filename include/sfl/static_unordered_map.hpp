...
#ifndef SFL_STATIC_UNORDERED_MAP_HPP_INCLUDED
#define SFL_STATIC_UNORDERED_MAP_HPP_INCLUDED
...
public:
    ...
    static_unordered_map(const Hash& hash)
    ...
    static_unordered_map(InputIt first, InputIt last)
        : hash_table_(StaticBucketCount)
    {
        insert(first, last);
    }
    ...
    template <typename InputIt,
              sfl::dtl::enable_if_t<sfl::dtl::is_input_iterator<InputIt>::value>* = nullptr>
    static_unordered_map(InputIt first, InputIt last, const Hash& hash)
        : hash_table_(StaticBucketCount, hash)
    {
        insert(first, last);
    }
    ...
    template <typename InputIt,
              sfl::dtl::enable_if_t<sfl::dtl::is_input_iterator<InputIt>::value>* = nullptr>
    static_unordered_map(InputIt first, InputIt last, const Hash& hash, const KeyEqual& equal)
        : hash_table_(StaticBucketCount, hash, equal)
    {
        insert(first, last);
    }

    static_unordered_map(std::initializer_list<value_type> ilist)
        : static_unordered_map(ilist.begin(), ilist.end())
    {}

    static_unordered_map(std::initializer_list<value_type> ilist, const Hash& hash)
        : static_unordered_map(ilist.begin(), ilist.end(), hash)
    {}

    static_unordered_map(std::initializer_list<value_type> ilist, const Hash& hash, const KeyEqual& equal)
        : static_unordered_map(ilist.begin(), ilist.end(), hash, equal)
    {}
    ...
    static_unordered_map(static_unordered_map&& other)
    ...
    static_unordered_map(sfl::from_range_t, Range&& range)
        : hash_table_(StaticBucketCount)
    {
    ...
    template <sfl::dtl::container_compatible_range<value_type> Range>
    static_unordered_map(sfl::from_range_t, Range&& range, const Hash& hash)
        : hash_table_(StaticBucketCount, hash)
    {
        insert_range(std::forward<Range>(range));
    }
    ...
    template <sfl::dtl::container_compatible_range<value_type> Range>
    static_unordered_map(sfl::from_range_t, Range&& range, const Hash& hash, const KeyEqual& equal)
        : hash_table_(StaticBucketCount, hash, equal)
    {
        insert_range(std::forward<Range>(range));
    }
    ...
    static_unordered_map& operator=(static_unordered_map&& other)
    {
        hash_table_.assign_move(other.hash_table_, sfl::dtl::hash_table_unique_t());
        return *this;
    }

    static_unordered_map& operator=(std::initializer_list<value_type> ilist)
    {
    ...
    void swap(static_unordered_map& other)
    {
    ...
    T& at(const Key& key)
    {
        ...
        if (it == end())
        {
            sfl::dtl::throw_out_of_range("sfl::static_unordered_map::at");
        }
...
private:
    ...
    template <typename K2, typename T2, std::size_t N2, std::size_t M2, typename H2, typename E2>
    friend bool operator==(const static_unordered_map<K2, T2, N2, M2, H2, E2>& x, const static_unordered_map<K2, T2, N2, M2, H2, E2>& y);
...
template <typename K, typename T, std::size_t N, std::size_t M, typename H, typename E>
SFL_NODISCARD
bool operator==
(
    const static_unordered_map<K, T, N, M, H, E>& x,
    const static_unordered_map<K, T, N, M, H, E>& y
)
{
...
void swap
(
    static_unordered_map<K, T, N, M, H, E>& x,
    static_unordered_map<K, T, N, M, H, E>& y
)
{
    x.swap(y);
}

template <typename K, typename T, std::size_t N, std::size_t M, typename H, typename E, typename Predicate>
typename static_unordered_map<K, T, N, M, H, E>::size_type
    erase_if(static_unordered_map<K, T, N, M, H, E>& c, Predicate pred)
{
...
#endif // SFL_STATIC_UNORDERED_MAP_HPP_INCLUDED
